...
if (extend) {
  if ((sum(toupper(GTF3$annotationType) %in% c("TRANSCRIPT", "CDS", "MRNA") & GTF3$gene_type == "protein_coding") + 1) / length(toupper(GTF3$annotationType) %in% c("TRANSCRIPT", "CDS", "MRNA")) < 0.20) {
