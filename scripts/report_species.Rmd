...
config <- read_config(file.path(getwd(), 'requirements_file/config_file.conf'))
...
params_df %>%
  kbl(caption = "Parameters", row.names = FALSE) %>%
...
plot_meta <- plot_meta[!plot_meta$subtypes %in% 'Undefined',]
p <- plotly::plot_ly(
  data = plot_meta,
  x = ~fUMAP1,
  y = ~fUMAP2,
  ...
  plotly::layout(
    ...
    margin = list(t = 60, b = 60, l = 60, r = 120),  
    autosize = TRUE
  ) %>%
  plotly::config(displayModeBar = FALSE)


p <- plotly::layout(p, height = 700, width = 1350)
...
cat('The analysis was performed through the non-profit and open-source pipeline JSEQ_scRNAseq©. ')
cat('It is part of the JBioSystem© project, which develops complex bioinformatics analysis software. ')
cat('Remember, in silico analysis must be validated in vitro or in vivo. For more information, visit GitHub or contact us. ')
cat('JSEQ_scRNAseq© was created at the Institute of Bioorganic Chemistry, Polish Academy of Sciences.')
```
