...
DQC <- ggplot() +
  ...
  geom_vline(xintercept = down_tr, color = "green") +
  annotate("text", x = down_tr - 40, y = as.integer(round(max(QC_UMI$nGenes) / 1.3)), label = as.character(paste0("> ", as.character(round(down_tr)))), angle = 90) +
  geom_vline(xintercept = n_gen, color = "red") +
  annotate("text", x = n_gen - 40, y = as.integer(round(max(QC_UMI$nGenes) / 1.3)), label = as.character(paste0("< ", as.character(round(n_gen)))), angle = 90) +
...
sc_project <- CSSG.toolkit::naming_genes_selection(sc_project,
  type = "primary", top_n = top_m,
...
sc_project <- CSSG.toolkit::subclass_naming(
...
CSSG.toolkit::marker_heatmap(sc_project,
  type = "subclasses", markers = markers,
  angle_col = 270,
  fontsize_row = 15,
  fontsize_col = 15,
  font_labels = 15,
...
if (tolower(heterogeneity) == "var") {
  sc_project <- CSSG.toolkit::heterogeneity_select_variance(
    sc_project = sc_project,
  ...
  sc_project <- CSSG.toolkit::heterogeneity_select_specificity(
    sc_project = sc_project,
    type = "primary",
    heterogeneity_factor = s_factor,
    p_val = m_val,
...
sc_project <- CSSG.toolkit::CSSG_markers(
  sc_project = sc_project,
...
thr_data <- CSSG.toolkit::bin_cell_test(
  p_val = p_bin,
...
rmarkdown::render(
  input = file.path(getwd(), "scripts/report_species.Rmd"),
