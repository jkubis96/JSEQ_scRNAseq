...
meta_data$barcodes <- rownames(meta_data)
UMAP_coordinates <- as.data.frame(UMI@reductions$umap@cell.embeddings)
meta_data$UMAP1 <- UMAP_coordinates$umap_1
meta_data$UMAP2 <- UMAP_coordinates$umap_2
...
height <- 7 + (length(unique(Idents(UMI))))/5

svg(file.path(OUTPUT, "subclasses_composition.svg"), width = 10, height = height)
threshold
dev.off()
...
meta_data$subclass <- sc_project@names$subclass
meta_data$subtypes <- sc_project@names$subtypes
