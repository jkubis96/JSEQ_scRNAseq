---
title: "JSEQ_scRNAseq - manual analysis report"
date: "`r Sys.Date()`"
output:
  html_document:
    theme: flatly       
...
config <- read_config("../../../requirements_file/config_file.conf")

results_dir <- file.path(project_path, "manual_results")
...
if (chceck_fq) {
  show_image(file.path(getwd(), "figures/scRNAmetrics.jpeg"), "## Read distribution across genomic features")
}
```

```{r echo=FALSE, out.width = '100%', fig.align="center", results='asis'}
if (chceck_fq) {
  show_image(file.path(getwd(), "figures/expect_whitelist_cell_barcode_counts.png"), "## Count (UMI) per barcode (cell)")
}
```

```{r echo=FALSE, out.width = '100%', fig.align="center", results='asis'}
if (chceck_fq) {
  show_image(file.path(getwd(), "figures/expect_whitelist_cell_barcode_knee.png"), "## Cell barcode knee plot")
}
```
